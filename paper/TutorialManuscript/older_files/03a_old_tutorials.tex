...
We will make use of that program in the Analysis Tutorials (Section \ref{tut:analysis-adv}).
...
Users should have completed the \textbf{Basic Tutorial} (\textbf{Section \ref{tut:nacl-basic}}) and have a potential progress coordinate in mind for their system of interest. 
...
\textbf{Preparing the Simulation Environment.} See the corresponding subsection in the \textbf{Basic Tutorial}.
...
The \verb|get_pcoord.sh| script will calculate the initial progress coordinates using AmberTools’ \verb|cpptraj| program from within the script, as opposed to reading the value from an external file as in the \textbf{Basic Tutorial}.
...
We focus on the p53 peptide system described above in the \textbf{Intermediate Tutorial} (\textbf{Section \ref{tut:p53-int}}) in which the progress coordinate is the C\textsubscript{$\alpha$} RMSD of the peptide from its folded, $\alpha$-helical conformation
